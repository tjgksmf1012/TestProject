...
**현재 상태: 연구·설계 단계. 구현 코드 없음.**
이 저장소는 "무엇을 어떻게 만들 것인가"를 검증하고 기록하는 곳입니다.
...
4. **16GB VRAM에 ASR + 화자분리 + LLM을 동시에 못 올립니다.**
...
6. **RTX 5080은 Blackwell(sm_120)입니다.** PyTorch **2.7.0+ / cu128** 이 최소 조건이고,
   기본 인덱스로 설치하면 안 됩니다. 환경 구축을 0주차로 배치하세요.
...
| [02. 모델 선정과 VRAM 예산](docs/02-모델-선정과-VRAM-예산.md) | RTX 5080 16GB에서 실제로 무엇이 돌아가는가 |
