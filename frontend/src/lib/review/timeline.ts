 ...
 * ## ⚠️ **회의 상태를 봅니다** (결함 346)
 *
 * 예전에는 상태를 안 받고 한 문장을 돌려줬습니다 — 「녹음이 아직
 * 처리되지 않았거나, 녹음 없이 열린 회의입니다」. 언제나 참인 **양다리**
 * 라서 틀린 말은 아니지만, 화면은 상태를 쥐고 있으면서 안 쓴 것입니다.
 *
 * 그리고 처리에 **실패한** 회의에게 「아직 처리되지 않았거나」는 「기다리면
 * 온다」로 읽힙니다 — 결함 284 가 바로 옆 요약 칸에서 고친 그 병이
 * 타임라인 칸에 남아 있었습니다. 검토 화면의 빈 상자는 셋이고, 그때
 * 고쳐진 것은 둘뿐이었습니다 (결함 301 — 옆 갈래를 세십시오).
 */
export function emptyTimelineNote(status: string | null | undefined): string {
  switch (status) {
    case 'pending':
      return '아직 녹음하지 않은 회의입니다 — 녹음을 마치면 발화가 여기 쌓입니다';
    case 'queued':
      return '녹음은 끝났고 처리 차례를 기다리는 중입니다 — 아직 시작하지 않았습니다';
    case 'processing':
      return '녹음을 처리하는 중입니다 — 끝나면 발화가 여기 쌓입니다';
    case 'failed':
      return '처리에 실패해 발화가 없습니다 — 로비에서 다시 처리할 수 있습니다';
    case 'needs_review':
    case 'confirmed':
      // 처리는 끝났는데 발화가 0건 — **고장이 아니라 결과**입니다.
      // 「기다리세요」라고 하면 오지 않을 것을 기다립니다.
      return '처리는 끝났는데 기록된 발화가 없습니다 — 소리가 짧거나 알아듣지 못했을 수 있습니다';
    default:
