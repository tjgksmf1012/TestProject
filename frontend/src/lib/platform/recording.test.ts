...
describe('사람에게 하는 말', () => {
  ...
  it('⭐ 폰 이야기를 하지 않는다', () => {
    ...
    //
    // ⛔ **이 자는 이 함수만 봅니다.** 오래도록 「폰 이야기 금지」를 재는
    //    자가 여기 하나뿐이라, 같은 낱말이 설치 어휘(`pwa/install.ts`)와
    //    마크업에 그대로 남아 데스크톱 홈 화면이 「홈 화면에 추가」 단추를
    //    그리고 있었습니다(결함 422 — 자가 아니라 **걷는 자리**가 좁았던
    //    경우, 결함 286 의 모양). 그쪽은 `pwa/install.test.ts` 가 잽니다.
    //    ⚠️ 사람에게 보이는 안내를 새로 만들면 **그 자리에도** 같은 자를
    //    대십시오 — 이 파일 하나가 제품 전체를 지키지 못합니다.
    for (const s of ALL) {
      const text = describeRecordingSafety(s);
