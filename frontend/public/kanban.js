...
async function fillPanel(apiBase2, projectId2) {
  ...
  host.innerHTML = `<p class="ctx-head">${escapeHtml(panelHeading(members2.length))}</p>` + rows + (note === null ? "" : `<p class="ctx-note">${escapeHtml(note)}</p>`);
}
