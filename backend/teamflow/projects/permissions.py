...
class Action(StrEnum):
    ...
    **바꾸는 것**입니다.
    ...
    #: 팀 **전원의** 기여도를 확정하기 (결함 392)
    CONFIRM_CONTRIBUTIONS = "confirm_contributions"
...
_ALLOWED: dict[Action, ProjectRole] = {
    ...
    # ⚠️ 확정은 **팀 전원의 숫자를 한 번에 쓰는 일**입니다 (결함 392).
    #
    #    이 라우트는 오래도록 `_require_project_member` 만 봤습니다. 즉
    #    평범한 팀원이 자기 몫을 90%, 나머지를 5%씩으로 확정할 수 있었고
    #    (재현했습니다 — 201), 기록에는 그 사람 이름이 붙습니다.
    #
    #    이 저장소는 **더 작은 일**에 대해 이미 반대로 정해 뒀습니다:
    #    `set_my_role` 은 「역할은 가중치를 바꾸고, 가중치는 점수를
    #    바꿉니다. 남이 내 역할을 바꿀 수 있으면 그건 **남의 점수를 바꾸는
    #    일**입니다」라며 경로 자체를 `/members/me` 로 두어 남의 id 를 넣을
    #    자리를 없앴습니다. 이름을 바꾸는 것(`EDIT_PROJECT`)조차
    #    관리자입니다. 확정만 아무 문이 없던 것입니다.
    #
    #    쓰기 라우트 44개를 세어 보니 「팀원 누구나」이면서 **남의 숫자**를
    #    쓰는 것은 이것 하나였습니다 — 나머지 셋은 경로가 `/me` 입니다.
    #
    # ⚠️ **불변식 ④를 뒤집는 것이 아닙니다.** 「팀이 합의해 확정한다」는
    #    그대로이고, 여기서 정하는 것은 **합의를 기록으로 남기는 손이
    #    누구인가**뿐입니다. 팀원도 되게 하려면 이 한 줄을 `MEMBER` 로
    #    바꾸면 됩니다 — 그때는 「남의 숫자를 쓰는 일을 누구나 한다」를
    #    근거와 함께 적어 두십시오.
    Action.CONFIRM_CONTRIBUTIONS: ProjectRole.ADMIN,
}
