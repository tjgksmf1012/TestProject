...
def speaking_ratios(stats: list[TrackStats]) -> list[float | None]:
    """각 트랙이 **팀 전체 발언 시간에서 차지하는 몫** (0~1).

    요구사항 정의서 AI-AUDIO-005 "발언 비중 계산".

    ⚠️ **아무도 말하지 않았으면 `None` 입니다. 0.0 이 아닙니다.**
    분모가 0이면 비중이라는 것이 존재하지 않습니다 — 그때 0.0 을 돌려주면
    "다들 0% 말했다" 는 **잰 값**처럼 보이는데, 실제로는 잴 것이 없었던
    것입니다. 이 구분이 이 저장소의 세 번째 불변식입니다.

    ⚠️ **이 값을 순위로 그리지 마십시오.**

    요구사항 정의서 AI-AUDIO-005 의 예시는 이렇게 생겼습니다:

        윤식 32% / 민수 27% / 지연 25% / 철수 16%

    내림차순으로 늘어놓은 그 모양이 곧 리더보드입니다. 그런데 **같은
    문서가 두 곳에서 반대로 말합니다** — AI-REVIEW-007 은 "발언 비중은
    기여도를 판단하는 기준으로 직접 사용하지 않는다", NFR-005 는 단정적
    평가를 금지하며 올바른 예로 "이번 회의에서 철수의 발언 비중은 전체
    발언의 8%였다" 를 듭니다. 즉 요구는 **값을 만들라**는 것이지 **줄을
    세우라**는 것이 아닙니다. 예시의 생김새만 리더보드였습니다.

    그래서 이 함수는 **받은 순서(트랙 순서) 그대로** 돌려줍니다. 정렬하지
    않습니다 — 정렬해서 주면 부르는 쪽은 그게 뜻있는 순서라고 믿습니다.

    화면에 그릴 때 지킬 것 (`docs/05` §5 · `docs/20`):
      * 사람은 **이름 순**으로, 값은 **글자로**
      * 값을 같은 축 위에 폭이나 위치로 그리지 말 것 — 그 순간 순위표입니다
      * 기여도 점수에 넣지 말 것. 이건 **참고 표시**입니다 (`docs/05` §5 표)
    """
    total = sum(s.speaking_ms for s in stats)
    if total <= 0:
        # 아무도 말하지 않은 회의. 비중이 없는 것이지 0인 것이 아닙니다.
        return [None] * len(stats)
    return [s.speaking_ms / total for s in stats]
