...
#: 사고처럼 적힌 것을 보게 된다.
#:
#: ⚠️ **세 값 중 하나가 오래도록 비어 있었습니다** (결함 326). `consent_refused`
#: — ② 원본 보관을 **본인이 거부**해서 전사 직후 지운 것 — 만 문구가 없어서
#: `_DELETION_REASON_UNKNOWN` 으로 떨어졌고, 화면이 「(사유 미기록)」이라고
#: 적었습니다. **같은 응답의 바로 옆 칸**에 `"deleted_reason":
#: "consent_refused"` 가 들어 있는 채로입니다. 사유는 기록돼 있는데 없다고
#: 말한 것이고, 위 ⭐ 가 막으려던 바로 그 일 — 권리 행사가 사고처럼 적히는 것
#: — 이 났습니다. 아래 짝 가드가 이제 값마다 문구가 있는지 셉니다.
_DELETION_REASON_TEXT = {
    retention.REASON_CONSENT_REFUSED: (
        "본인이 원본 보관에 동의하지 않아 전사 직후 녹음 원본을 삭제했습니다. "
        "발언량을 측정할 수 없어 회의 기여도를 계산에서 제외했습니다"
    ),
