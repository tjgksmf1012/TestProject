...
#: 사람이 읽을 이름.
#:
#: ⚠️ **화면의 `lib/contribution/view.ts` 와 짝입니다.** 키가 어긋나면
#: `test_repo_integrity.py` 가 잡습니다 — 그런데 그 검사는 **키 집합만**
#: 봅니다. 서버에는 한국어 이름이 **아예 없었고**, 그래서 최종 보고서가
#: 이렇게 나갔습니다 (결함 291) —
#:
#:     가중치가 0이라 계산에서 빠진 영역: document, schedule, peer
#:
#: ⚠️ 위 줄은 **그때 나갔던 글자 그대로**입니다. 앞머리(「가중치가 0이라」)는
#: 그 뒤 결함 311 이 고쳤습니다 — 그 목록을 만드는 자는 가중치가 아니라
#: **팀의 활동량**이었습니다. 지금 코드에서 저 문장을 찾지 마십시오.
#:
#: 성적으로 이어질 수 있는 문서에서, 학생이 자기 계산에서 무엇이 빠졌는지
#: 읽을 수 없는 상태였습니다.
CATEGORY_LABEL: dict[Category, str] = {
    ...
    Category.SCHEDULE: "일정 준수",
    Category.PEER: "동료 평가",
}


def describe_category(key: str) -> str:
    """기여 영역의 한국어 이름.

    ⚠️ 모르는 값은 **그대로 돌려줍니다** — 지어낸 한국어보다 영어 식별자가
    정직합니다. 화면의 `describeCategory` 와 같은 규칙입니다.
    """
    try:
        return CATEGORY_LABEL[Category(key)]
    except ValueError:
        return key
